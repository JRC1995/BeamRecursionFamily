...
SEED = 101
dev_keys = ["normal"]
test_keys = ["normal", "LRA", "iid_arg", "ood_10arg", "ood_15arg",
             "200_300", "300_400", "400_500", "500_600", "600_700", "700_800", "800_900", "900_1000"]
np.random.seed(SEED)
random.seed(SEED)
max_seq_len = 100

train_path = Path('../data/listops/train_d20s.tsv')
test_path = {}
test_path["normal"] = Path('../data/listops/test_d20s.tsv')
test_path["LRA"] = Path('../data/listops/basic_test.tsv')
for key in test_keys:
    if key != "normal" and key != "LRA":
        test_path[key] = Path('../data/listops/test_{}.tsv'.format(key))

Path('../processed_data/listopsc/').mkdir(parents=True, exist_ok=True)

train_save_path = Path('../processed_data/listopsc/train.jsonl')
dev_save_path = {}
for key in dev_keys:
    dev_save_path[key] = Path('../processed_data/listopsc/dev_{}.jsonl'.format(key))
test_save_path = {}
for key in test_keys:
    test_save_path[key] = Path('../processed_data/listopsc/test_{}.jsonl'.format(key))

metadata_save_path = fspath(Path("../processed_data/listopsc/metadata.pkl"))
