...
train_path = Path('../data/listops_lra/basic_train.tsv')
dev_path = {}
dev_path["normal"] = Path('../data/listops_lra/basic_val.tsv')
test_path = {}
test_path["normal"] = Path('../data/listops_lra/basic_test.tsv')

Path('../processed_data/listops_lra_speed/').mkdir(parents=True, exist_ok=True)

train_save_path = Path('../processed_data/listops_lra_speed/train.jsonl')
dev_save_path = {}
for key in dev_keys:
    dev_save_path[key] = Path('../processed_data/listops_lra_speed/dev_{}.jsonl'.format(key))
test_save_path = {}
for key in test_keys:
    test_save_path[key] = Path('../processed_data/listops_lra_speed/test_{}.jsonl'.format(key))

metadata_save_path = fspath(Path("../processed_data/listops_lra_speed/metadata.pkl"))
...
def process_data(filename, update_vocab=True, skip_first_row=True, reverse=True):
    global labels2idx
...
for x, y in zip(train_sequences, train_labels):
    if len(x) >= 1500:
        print(len(x))
        train_sequences_.append(x)
        train_labels_.append(y)
        i += 1
        if i == 100:
            break

train_sequences, train_labels = train_sequences_, train_labels_
